 ...
 *
 * Runs the M178 dry-run harness across the canonical regression scenarios (full squad,
 * injury-thinned, invalid provider) from the shared fixtures and reports per-scenario pass/fail.
 * Real engines injected; no networking/persistence/AI/runtime activation.
 */
...
test('all scenarios pass', () => {
  const out = runBrainDryRunMatrix([createFullSquadScenario(), createInjuryThinnedScenario()], OPTIONS)
  ...
  const thinned = out.scenarios.find((s) => s.id === 'injury-thinned')
...
test('one scenario fails while later scenarios still run', () => {
  const out = runBrainDryRunMatrix([createFullSquadScenario(), createInvalidProviderScenario(), createInjuryThinnedScenario()], OPTIONS)
  assert.equal(out.total, 3)
  assert.equal(out.passed, 2)
  assert.equal(out.failed, 1)
  const invalid = out.scenarios.find((s) => s.id === 'invalid-provider')
  ...
  assert.equal(out.scenarios.find((s) => s.id === 'injury-thinned').ok, true)
})

test('order is preserved', () => {
  const out = runBrainDryRunMatrix([createFullSquadScenario(), createInvalidProviderScenario(), createInjuryThinnedScenario()], OPTIONS)
  assert.deepEqual(out.scenarios.map((s) => s.id), ['full-squad', 'invalid-provider', 'injury-thinned'])
})

test('expected-mismatch counts as failed without throwing', () => {
  const out = runBrainDryRunMatrix([{ ...createFullSquadScenario(), expected: { startingCount: 99 } }], OPTIONS)
...
test('deterministic — repeated runs are identical', () => {
  const scenarios = [createFullSquadScenario(), createInvalidProviderScenario(), createInjuryThinnedScenario()]
  const shape = (out) => out.scenarios.map((s) => ({ id: s.id, ok: s.ok, error: s.error, v: s.verification }))
  assert.deepEqual(shape(runBrainDryRunMatrix(scenarios, OPTIONS)), shape(runBrainDryRunMatrix(scenarios, OPTIONS)))
})
...
test('does not mutate scenario inputs', () => {
  const scenarios = [createFullSquadScenario(), createInvalidProviderScenario()]
...
test('malformed scenarios are rejected clearly', () => {
  ...
  assert.throws(() => runBrainDryRunMatrix([{ ...createFullSquadScenario(), expected: 'x' }], OPTIONS), TypeError)
  assert.throws(() => runBrainDryRunMatrix([createFullSquadScenario()], 'x'), TypeError)
})
...
test('the dry-run function is called exactly once per scenario (injected)', () => {
  ...
  const out = runBrainDryRunMatrix([createFullSquadScenario(), createInjuryThinnedScenario(), createInvalidProviderScenario()], OPTIONS, { runDryRun: fakeDryRun })
