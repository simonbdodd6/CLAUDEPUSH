...
const okPair = (id, codes) => ({ id, ok: true, diff: { summary: { changed: codes.length > 0, codes } }, diffView: { changed: codes.length > 0, changeCount: codes.length, codes }, error: null })
const failPair = (id, error) => ({ id, ok: false, diff: null, diffView: null, error })
...
const MATRIX = makeMatrix(
  [okPair('captain', ['CAPTAIN_CHANGED', 'PLAYER_PROMOTED']), okPair('same', []), failPair('bad', 'diff boom')],
  { changeCodeCounts: { CAPTAIN_CHANGED: 1, PLAYER_PROMOTED: 1 }, changedPairCount: 1, unchangedPairCount: 1 },
)
...
test('object format returns the normalized summary', () => {
  ...
  assert.deepEqual(out.pairs[0], { id: 'captain', ok: true, changed: true, changeCount: 2, codes: ['CAPTAIN_CHANGED', 'PLAYER_PROMOTED'], error: null })
  assert.deepEqual(out.pairs[1], { id: 'same', ok: true, changed: false, changeCount: 0, codes: [], error: null })
  assert.deepEqual(out.pairs[2], { id: 'bad', ok: false, changed: false, changeCount: 0, codes: [], error: 'diff boom' })
  assert.deepEqual(out.rollup, { changeCodeCounts: { CAPTAIN_CHANGED: 1, PLAYER_PROMOTED: 1 }, changedPairCount: 1, unchangedPairCount: 1 })
})
...
test('text format renders deterministic lines + rollup', () => {
  ...
  assert.equal(lines[1], 'captain ok=true changed=true changes=2 codes=CAPTAIN_CHANGED,PLAYER_PROMOTED')
  assert.equal(lines[2], 'same ok=true changed=false changes=0 codes=')
  assert.equal(lines[3], 'bad ok=false error="diff boom"')
  assert.equal(lines[4], 'rollup CAPTAIN_CHANGED=1 PLAYER_PROMOTED=1')
})

test('empty rollup renders a bare "rollup" line', () => {
  const m = makeMatrix([okPair('a', [])], { changeCodeCounts: {}, changedPairCount: 0, unchangedPairCount: 1 })
  assert.equal(summarizeBrainDryRunDiffMatrix(m, 'text').split('\n').at(-1), 'rollup')
})
...
test('missing rollup fields and diffView default safely', () => {
  const m = { total: 1, passed: 1, failed: 0, pairs: [{ id: 'x', ok: true }], rollup: {} }   // no diffView, empty rollup
  const out = summarizeBrainDryRunDiffMatrix(m, 'object')
  assert.deepEqual(out.pairs[0], { id: 'x', ok: true, changed: false, changeCount: 0, codes: [], error: null })
  assert.deepEqual(out.rollup, { changeCodeCounts: {}, changedPairCount: 0, unchangedPairCount: 0 })
})
