...
## 2026-06-14 — Publisher Adoption (M20 Phase 1) — "The Living Intelligence"

- Began feeding the intelligence chain with REAL activity by making two existing travel modules publish into Timeline (M12) and the Relationship Graph (M13) through optional injected publisher ports. No AI, recommendations, notifications, or execution added. Adopted exactly two modules, confirmed at review as the safest, highest-value boundaries: **trip-platform** and **itinerary-platform**.
- Architecture: publishers are INJECTED, never imported — `createTripPlatform`/`createItineraryPlatform` accept optional `{ timelinePublisher, relationshipPublisher }` (the timeline/graph platforms, or any object exposing `appendEvent`/`createRelationship`). When absent, behaviour is byte-for-byte identical to before (verified: all 29 prior trip+itinerary tests pass unchanged). Publishing is best-effort and ISOLATED — every publish is wrapped so a publisher failure can never fail the business operation. Only references (ids) are published; zero trip/itinerary business data is duplicated onto events or edges.
- Timeline events added — trip-platform: `trip_created`, `trip_updated`, `trip_cancelled`, `trip_completed`; itinerary-platform: `itinerary_created`, `itinerary_updated`, `activity_added`, `activity_removed`. Events use native M12 types where they exist (`trip_created`, `trip_updated`, `activity`) and the `custom` escape hatch otherwise, with the precise name always in `metadata.eventName` — so no change to the M12/M13 modules was required (boundary preserved).
- Relationship edges added — trip-platform: Traveller `owns` Trip, Trip `visited` Destination (VISITS→native `visited`); itinerary-platform: Trip `contains` Itinerary, Itinerary `contains` Activity (CONTAINS→`custom` + `metadata.relationshipName`), Traveller `planned` Activity, Activity `located_in` Destination (LOCATED_AT→native `located_in`). Trip CONTAINS Itinerary is published by itinerary-platform (which holds both ids), since trip-platform has no itinerary knowledge — documented placement decision.
- Idempotency: timeline events carry stable idempotency keys (one-time lifecycle events keyed by entity id; repeatable updates keyed by entity id + version/updatedAt so distinct updates create distinct events while re-runs are deduped); relationship edges rely on the graph's built-in (from,to,type) dedup. Duplicate-on-replay errors from either platform are swallowed by the isolation wrapper, so repeated execution never creates duplicate events or edges.
- Adoption strategy: phased and opt-in — the publisher ports default to off, so existing callers and the legacy app are unaffected; only a composition root that injects the timeline/graph turns publishing on. This keeps every architectural boundary from M7–M19 intact while letting the chain (context → insight → action-candidate → approval) finally run on genuine traveller behaviour.
- Files changed: `lib/trip-platform/service.js` (injected publisher ports + isolated publish hooks on create/update/dates/destination/visibility/complete/cancel); `lib/itinerary-platform/service.js` (injected publisher ports + hooks on create, activity add/remove, and a generic `itinerary_updated` via `commit`); new `test/travel-publisher-adoption.test.js` (11 tests); this `PROJECT_STATUS.md` entry. No other module touched; timeline/graph/identity unchanged.
- Verified: full `node --test` suite passes (641/641); travel platform suite 213/213; prior trip+itinerary tests 29/29 unchanged; decoupling asserted in-test (neither module imports timeline or relationship-graph). Tests cover: trip→timeline, trip→graph, itinerary→timeline, itinerary→graph, no-publishers legacy behaviour, isolated publisher failures, deterministic ids, no duplicate events, no duplicate edges, no direct imports, no business-data duplication.
- Risks: (1) `Trip VISITS Destination` and `Activity LOCATED_AT Destination` reference a destination id derived by slugifying the destination/destinationFocus name (e.g. `bali`), not the canonical destination-platform id — Phase 2 should resolve to canonical ids; (2) custom-typed events/edges (trip_cancelled/completed, itinerary_created/updated, contains) are queryable by `metadata.eventName`/`metadata.relationshipName` but not as first-class vocabulary — a small additive M12/M13 vocabulary extension would make them first-class if desired; (3) publish failures are silently swallowed (isolation) — there is no dead-letter/retry yet, so a persistently-down publisher loses events; (4) trip edges are published at create only (destination changes via `changeTripDestination` emit `trip_updated` but do not re-publish a new VISITS edge in Phase 1).
- Recommended M20 Phase 2: extend publisher adoption to the next highest-value modules — **destination-platform** (canonical destinations, enabling id resolution for the slug references above) and **travel-memory-platform** or **companion-discovery-platform** (memory_created / companion_match events + edges) — same opt-in injected-port pattern; optionally a tiny additive M12/M13 vocabulary extension to promote the custom-typed events/edges to first-class.

## 2026-06-14 — Publisher Adoption (M20 Phase 2) — Destinations + Memory

- Continued feeding the intelligence chain with real activity by adopting two more modules as publishers via optional injected ports: **destination-platform** and **travel-memory-platform**. No AI/recommendations/notifications/execution. Same pattern as Phase 1: injected `{ timelinePublisher, relationshipPublisher }`, publishers optional, publishing best-effort and ISOLATED, references-only, zero business-data duplication, full backward compatibility (verified: 29/29 prior destination+memory tests unchanged).
- Destination platform is now the canonical source for destination references: it publishes canonical destination nodes/edges keyed by `destinationId` (e.g. `dest_*`). Destinations are system-owned (not traveller-scoped), so their timeline events are written to a `system` identity timeline with `visibility: 'system'` (not on any real traveller's timeline).
- Phase 1 slug-derived destination id — FIXED (deterministically) and documented: when a destination is created/updated, destination-platform emits an alias edge `destination(destinationId) related_to destination(slug(name))` (RELATED_TO is symmetric/undirected). This bridges the name-slug ids that Phase 1 trip/itinerary publishing used (e.g. `bali`) to the canonical `dest_*` id, so existing Phase 1 references resolve to the canonical destination via one hop. (A full Phase-3 resolution would have trip/itinerary publish canonical ids directly.)
- Events emitted — destination: `destination_created` (native M12 `destination_added`), `destination_updated` (custom + metadata.eventName; covers update/activate/pause/close, carries status). memory: `memory_created` (native M12 `memory_created`), `memory_updated` (custom + metadata.eventName; on reinforce/correct/lock/decay via `commit`).
- Relationships emitted — destination: Destination `located_in` Parent (canonical ids, when parented); Destination `related_to` name-slug alias (the Phase 1 bridge). memory: Traveller `remembered` Memory (always); Memory `references` {destination|trip|activity} inferred from the memory key (destination value mapped via name-slug to bridge to canonical); Memory `references` {trip|destination|activity|itinerary} named in the memory's provenance sources. This realises traveller → memory → destination/trip where references exist.
- Idempotency: timeline events use stable keys (one-time create keyed by entity id; updates keyed by entity id + version/updatedAt); relationship edges rely on the graph's built-in (from,to,type) dedup. Duplicate-on-replay errors from either platform are swallowed by the isolation wrapper, so repeated execution never creates duplicate events or edges (verified).
- Files changed: `lib/destination-platform/service.js` (injected ports + isolated publish hooks on create/update/activate/pause/close + parent & alias edges); `lib/travel-memory-platform/service.js` (injected ports + hooks on persistNew = memory_created + edges, and commit = memory_updated); new `test/travel-publisher-adoption-phase2.test.js` (11 tests); this `PROJECT_STATUS.md` entry. No other module touched; timeline/graph unchanged.
- Verified: full `node --test` suite passes (652/652); travel platform suite 224/224; prior destination+memory tests 29/29 unchanged; decoupling asserted in-test (neither module imports timeline or relationship-graph). Tests cover: destination→timeline, destination→graph (parent + alias bridge), memory→timeline (created/updated), memory→graph (remembered + references via key and via provenance), no-publishers legacy behaviour, isolated publisher failures, deterministic idempotency, no duplicate events/edges, no direct imports, no business-data duplication.
- Risks: (1) the name-slug alias is a bridge, not a true resolution — two real destinations with the same slugified name would alias-collide; Phase 3 should have publishers emit canonical ids directly; (2) destination events live on a single `system` timeline that will grow unbounded — fine in-memory, needs partitioning/retention in production; (3) memory key→entity inference is heuristic (only keys `destination`/`trip`/`activity`), so memories with other keys link to the traveller but to no entity; (4) custom-typed events/edges remain queryable only via metadata until an optional additive M12/M13 vocabulary extension; (5) swallowed publish failures still have no dead-letter/retry.
- Recommended M20 Phase 3 / M21: (a) have trip-platform & itinerary-platform publish canonical destination ids (resolved through an injected destination-resolver port) to retire the slug bridge; and/or (b) adopt **companion-discovery-platform** (companion_match events + Traveller connected_to Traveller edges) and **activity-platform** (canonical activity ids to retire activity-id slugging); optionally promote the custom-typed timeline/graph vocabulary to first-class via a small additive M12/M13 change.
