...
function inBrain(x, y, z) {
  // Main cerebrum: elongated front-to-back (z), with temporal widening
  const ax = x / 1.20
  const ay = (y + 0.06) / 0.88
  const az = z / 1.10  // elongated front-back
  const temporalWiden = Math.max(0, 0.18 * (1 - ay * ay * 1.8))
  const r2 = (ax / (1 + temporalWiden)) ** 2 + ay ** 2 + az ** 2

  // Longitudinal fissure: carve a groove down the center top
  const inFissure = Math.abs(x) < 0.06 && y > 0.05 && r2 < 0.92

  if (r2 < 1.0 && !inFissure) return true

  // Cerebellum: posterior-inferior lobe
  const r2c = (x / 0.55) ** 2 + ((y + 0.48) / 0.32) ** 2 + ((z - 0.75) / 0.45) ** 2
  return r2c < 1.0
}

// ─── Cognitive subsystems (intelligence layers) ──────────────────
// Each particle belongs to one lobe. Clicking dives into that layer.
const LOBES = [
...
function classifyLobe(x, y, z) {
  ...
  if (inCereb)                 return 5  // cerebellum
  if (z < -0.35)               return 1  // frontal
  if (Math.abs(x) > 0.72)      return 2  // temporal
  if (y > 0.25)                return 3  // parietal
  if (z > 0.45)                return 4  // occipital
  return 0                                // executive core
}

function buildParticles(n) {
  const pos = [], col = [], sz = [], ph = [], pr = [], lob = []
  // Centroid accumulators per lobe
...
// ─── Precompute at module load (runs once) ───────────────────────
...
uniform float uFocusLobe;   // -1 = none, else lobe id
uniform float uFocusMix;    // 0..1 transition strength
...
void main() {
  ...
  // Layer focus: is this particle in the focused subsystem?
  float isFocus = step(abs(aLobe - uFocusLobe), 0.5);  // 1 if in focus lobe
  // When focused, non-focus particles dim & shrink; focus particles grow & brighten
  float dim   = mix(1.0, mix(0.12, 1.35, isFocus), uFocusMix);
  vDim = mix(1.0, isFocus, uFocusMix);   // 1 for focus, →0 for others
  ...
  // Factor 18 → at dist 5: size=1→3.6px, size=3→10.8px
...
void main() {
  ...
  // Subtle HDR boost only at the very center for bloom pickup
  vec3 col = vColor;
  col += vColor * core * 0.8;
...
void main() {
  ...
  float a    = core + halo;
  if (a < 0.01) discard;
  // HDR white-blue — blooms strongly
...
function Particles({ firingRef, activityRef, focusRef, hoverRef }) {
  ...
  useFrame(({ camera, mouse }, delta) => {
    ...
    // Animate layer-focus transition
    ...
    // Hover → fire neurons + remember hovered index/lobe for click
    if (pts.current) {
...
function Signals({ activityRef }) {
  ...
  useFrame((_, delta) => {
    ...
    // Spawn signals at rate proportional to brain activity
...
// ═══════════════════════════════════════════════════════════════════
// BRAIN GROUP  (spring rotation + autonomous activity + layer diving)
// ═══════════════════════════════════════════════════════════════════

function BrainGroup({ firingRef, activityRef, focusRef, hoverRef, setFocusLobe }) {
  ...
  // Ignite a burst of firing around a region of particles
  ...
  useEffect(() => {
    ...
    const onWheel = e => {
      ...
      // Scrolling back out surfaces from a focused layer
    ...
    const onClick = () => {
      ...
      if (hovIdx >= 0 && hovLobe >= 0) {
        // Clicked a node → dive into / toggle that intelligence layer
        const f = focusRef.current
        if (f.target === hovLobe) {
          f.target = -1
          setFocusLobe(-1)
          ...
          zoomRef.current = 2.7
          // Find a particle index inside that lobe to ignite around
          let seed = hovIdx
          ignite(seed, 220, 90)
        }
        ignite(hovIdx, 60, 40)
      } else {
        // Clicked empty space → surface back out
  ...
  useFrame(({ clock, camera }, delta) => {
    ...
    if (groupRef.current) {
      ...
      // Free rotation (surface mode)
      const freeY = t * 0.042 + s.rx * 0.38
      const freeX = s.ry * 0.20

      // Focused rotation: face the chosen lobe centroid toward the camera
      let focY = freeY, focX = freeX, offY = 0
      if (lobe >= 0) {
        const [cx, cy, cz] = BD.centroids[lobe]
        focY = -Math.atan2(cx, cz) + s.rx * 0.18   // bring centroid to +z
        focX = Math.atan2(cy, Math.hypot(cx, cz)) * 0.6 + s.ry * 0.12
        offY = -cy * SCALE * 0.9                     // recenter vertically
      }
    ...
    // Activity decays toward 1; a focused layer hums hotter
    const floor = focusRef.current.target >= 0 ? 1.6 : 1.0
    activityRef.current = Math.max(floor, activityRef.current - delta * 0.65)

    // Autonomous firing bursts (simulate background cognition)
    if (Math.random() < delta * 0.28) {
      activityRef.current = Math.min(2.8, activityRef.current + Math.random() * 1.0)
...
function Scene({ firingRef, activityRef, focusRef, hoverRef, setFocusLobe }) {
  return (
    <>
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ marginTop: 8, color: 'rgba(60, 255, 160, 0.65)' }}>
      ...
      <div style={{
        ...
        <div style={{ marginTop: 5, color: 'rgba(90, 255, 160, 0.55)' }}>v2.0 · Live</div>
      </div>
      ...
      <div style={{
        ...
        color: 'rgba(70, 150, 255, 0.28)',
        fontSize: 8, letterSpacing: '0.14em',
        textAlign: 'right', pointerEvents: 'none',
        textTransform: 'uppercase', lineHeight: 2.1,
      }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: 'rgba(60, 255, 140, 0.8)',
          boxShadow: '0 0 8px rgba(60, 255, 140, 0.6)',
          animation: 'pulse-hud 2s ease-in-out infinite',
        }} />
        <span style={{
          ...mono,
          fontSize: 9, color: 'rgba(60, 255, 140, 0.55)',
      ...
      {/* Center-bottom: ACTIVE LAYER panel — appears when diving into a subsystem */}
      <div style={{
        ...mono,
      ...
      <style>{`
        ...
        @keyframes boot-fade {
          ...
          18%  { opacity: 1; }
          78%  { opacity: 1; letter-spacing: 0.32em; }
          100% { opacity: 0; letter-spacing: 0.32em; }
        }
      `}</style>
    </>
  )
