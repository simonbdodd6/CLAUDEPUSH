...
export function createTripPlatform(options = {}) {
  ...
  // references (ids) are published — never trip business data.
  ...
  // Native M12 event type per published event name (others use the `custom`
  // escape hatch with the precise name carried in metadata.eventName).
  const TRIP_TIMELINE_TYPE = {
    trip_created: 'trip_created',
    trip_updated: 'trip_updated',
    trip_cancelled: 'custom',
    trip_completed: 'custom',
  };
  ...
  async function publishTripEvent(trip, eventName) {
    if (!timelinePublisher) return;
    const isUpdate = eventName === 'trip_updated';
    await safePublish(() => timelinePublisher.appendEvent({
      travellerIdentityId: trip.ownerIdentityId,
      tripId: trip.tripId,
      eventType: TRIP_TIMELINE_TYPE[eventName] ?? 'custom',
      sourcePlatform: 'trip-platform',
      sourceEntityId: trip.tripId, // reference only — never trip data
      timestamp: trip.updatedAt ?? trip.createdAt,
      importance: eventName === 'trip_cancelled' ? 'high' : 'normal',
      metadata: { eventName, status: trip.status },
      // Lifecycle events are one-time; updates are keyed by updatedAt so distinct
      // updates create distinct events while re-runs stay idempotent.
      idempotencyKey: isUpdate
        ? `trip-platform:trip_updated:${trip.tripId}:${trip.updatedAt}`
        : `trip-platform:${eventName}:${trip.tripId}`,
    }));
  }

  async function publishTripRelationships(trip) {
    if (!relationshipPublisher) return;
    // Traveller OWNS Trip
    await safePublish(() => relationshipPublisher.createRelationship({
      from: { type: 'traveller', id: trip.ownerIdentityId },
      to: { type: 'trip', id: trip.tripId },
      relationshipType: 'owns',
    }));
    // Trip VISITS Destination (mapped to the graph's native `visited`)
    if (trip.destination) {
      await safePublish(() => relationshipPublisher.createRelationship({
        from: { type: 'trip', id: trip.tripId },
        to: { type: 'destination', id: slug(trip.destination) },
        relationshipType: 'visited',
        metadata: { via: 'trip-platform' },
      }));
    }
  }
  ...
  async function createTrip(input = {}, actor = { id: input.ownerIdentityId, type: 'TRAVELLER' }) {
    ...
    await publishTripEvent(trip, 'trip_created');
    await publishTripRelationships(trip);
    return clone(trip);
  }
  ...
  async function updateTrip(tripId, patch = {}, actor) {
    ...
    await publishTripEvent(updated, 'trip_updated');
  ...
  async function completeTrip(tripId, actor) {
    const updated = await transitionTrip(tripId, TRIP_STATUS.COMPLETED, actor, TRIP_AUDIT_ACTIONS.TRIP_COMPLETED);
    await publishTripEvent(updated, 'trip_completed');
    return updated;
  }

  async function cancelTrip(tripId, actor, reason = null) {
    const updated = await transitionTrip(tripId, TRIP_STATUS.CANCELLED, actor, TRIP_AUDIT_ACTIONS.TRIP_CANCELLED, { reason });
    await publishTripEvent(updated, 'trip_cancelled');
    return updated;
  }
