...
// This module's field list intentionally omits gps/geo (its original behaviour);
// preserved here and passed to the kernel guard so behaviour is unchanged.
...
// Backed by the platform kernel's shallow guard; preserves this module's own
// validationError, label default, and (gps/geo-free) field list.
...
export function createTravelMemoryPlatform(options = {}) {
  ...
  // best-effort and ISOLATED. Only references (ids) are published — never memory
  // business data.
  ...
  // Memory keys that reference another graph entity, so traveller → memory →
  // destination/trip/activity edges can be drawn where references exist.
  const MEMORY_KEY_ENTITY_TYPE = { destination: 'destination', trip: 'trip', activity: 'activity' };
  const SNAPSHOT_ENTITY_TYPES = ['trip', 'destination', 'activity', 'itinerary'];
  ...
  async function publishMemoryEvent(memory, eventName) {
    if (!timelinePublisher) return;
    const isUpdate = eventName === 'memory_updated';
    await safePublish(() => timelinePublisher.appendEvent({
      travellerIdentityId: memory.travellerIdentityId,
      eventType: eventName === 'memory_created' ? 'memory_created' : 'custom',
      sourcePlatform: 'travel-memory-platform',
      sourceEntityId: memory.memoryId, // reference only
      timestamp: memory.updatedAt ?? memory.lastConfirmed ?? memory.createdAt,
      metadata: { eventName, key: memory.key, polarity: memory.polarity, origin: memory.origin },
      idempotencyKey: isUpdate
        ? `travel-memory-platform:memory_updated:${memory.memoryId}:${memory.version}`
        : `travel-memory-platform:memory_created:${memory.memoryId}`,
    }));
  }

  async function publishMemoryEdges(memory) {
    if (!relationshipPublisher) return;
    // Traveller REMEMBERED Memory
    await safePublish(() => relationshipPublisher.createRelationship({
      from: { type: 'traveller', id: memory.travellerIdentityId },
      to: { type: 'memory', id: memory.memoryId },
      relationshipType: 'remembered',
    }));
    // Memory REFERENCES a destination/trip/activity inferred from its key.
    const keyEntityType = MEMORY_KEY_ENTITY_TYPE[memory.key];
    if (keyEntityType && memory.value) {
      const refId = keyEntityType === 'destination' ? memorySlug(memory.value) : String(memory.value);
      await safePublish(() => relationshipPublisher.createRelationship({
        from: { type: 'memory', id: memory.memoryId },
        to: { type: keyEntityType, id: refId },
        relationshipType: 'references',
        metadata: { via: 'memory_key' },
      }));
    }
    // Memory REFERENCES entities named in its provenance snapshots.
    for (const source of memory.sources ?? []) {
      if (source && SNAPSHOT_ENTITY_TYPES.includes(source.snapshotType) && source.snapshotId) {
        await safePublish(() => relationshipPublisher.createRelationship({
          from: { type: 'memory', id: memory.memoryId },
          to: { type: source.snapshotType, id: String(source.snapshotId) },
          relationshipType: 'references',
          metadata: { via: 'memory_source' },
        }));
      }
    }
  }
  ...
  async function commit(memory, { action, details = {}, label }) {
    ...
    await publishMemoryEvent(memory, 'memory_updated');
    return clone(memory);
  }
  ...
  async function persistNew(memory, action, details) {
    ...
    await publishMemoryEvent(memory, 'memory_created');
    await publishMemoryEdges(memory);
    return clone(memory);
  }
