...
function assertNoLocationDeep(value, label) {
  if (Array.isArray(value)) { value.forEach(v => assertNoLocationDeep(v, label)); return; }
  if (value && typeof value === 'object') {
    for (const [key, nested] of Object.entries(value)) {
      if (EXACT_LOCATION_FIELDS.includes(key)) {
        throw validationError(`${label} must not include exact location field: ${key}`, { field: key });
      }
      assertNoLocationDeep(nested, label);
    }
  }
}
