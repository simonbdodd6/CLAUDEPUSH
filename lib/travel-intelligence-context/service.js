  ...
  if (value && typeof value === 'object') {
    for (const [key, nested] of Object.entries(value)) {
      if (EXACT_LOCATION_FIELDS.includes(key)) {
        throw validationError(`Context output must not include exact location field: ${key}`, { field: key });
      }
...
export function createTravelIntelligenceContext(options = {}) {
  ...
  async function buildContextSnapshot(travellerIdentityId, viewOptions = {}) {
    ...
    context.contextVersion = `${CONTEXT_VERSION_PREFIX}:${createHash('sha256').update(signature).digest('hex').slice(0, 16)}`;
