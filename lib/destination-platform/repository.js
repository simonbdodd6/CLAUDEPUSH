...
export class InMemoryDestinationRepository {
  ...
  async listChildren(parentDestinationId) {
    return [...this.destinations.values()]
      .filter(destination => destination.parentDestinationId === parentDestinationId)
      .map(clone)
      .sort((a, b) => a.name.localeCompare(b.name));
  }

  async listActiveChildren(parentDestinationId) {
    return [...this.destinations.values()]
      .filter(destination => destination.parentDestinationId === parentDestinationId)
      .filter(destination => destination.status === 'active')
      .map(clone)
