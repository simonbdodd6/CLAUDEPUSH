...
const PIPELINE_JSON = path.join(ROOT, 'qa/history/pipeline-latest.json');
...
// ─── Load pipeline metadata (written by qa/github-agent.js) ──────────────────
const pipeline = (() => {
  if (!fs.existsSync(PIPELINE_JSON)) return null;
  try { return JSON.parse(fs.readFileSync(PIPELINE_JSON, 'utf8')); } catch { return null; }
})();
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Pipeline panel */
    .pipeline { padding: 20px 32px; border-top: 1px solid var(--border); background: var(--surface); }
    .pipeline-title { font-size: 0.82rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: var(--muted); margin-bottom: 12px; }
    .pipeline-grid { display: flex; flex-wrap: wrap; gap: 10px; }
    .pipeline-doc { background: var(--bg); border: 1px solid var(--border); border-radius: 6px; padding: 8px 14px; font-size: 0.8rem; display: flex; align-items: center; gap: 8px; }
    .pipeline-doc a { color: var(--text); font-weight: 500; }
    .pipeline-doc a:hover { color: var(--accent); }
    .pipeline-doc .pdesc { color: var(--muted); font-size: 0.73rem; }
    .pipeline-meta { margin-top: 10px; font-size: 0.75rem; color: var(--muted); }
    .pipeline-branch { font-family: 'SF Mono', monospace; color: var(--accent); font-size: 0.78rem; }
...
${pipeline ? `
<div class="pipeline">
  <div class="pipeline-title">⚡ Regression Pipeline — last run ${pipeline.runAt?.slice(0,19).replace('T',' ')} UTC</div>
  <div class="pipeline-grid">
    <div class="pipeline-doc"><span>📋</span><div><a href="../../INVESTIGATION_REPORT.md" target="_blank">INVESTIGATION_REPORT.md</a><div class="pdesc">Root cause · component · confidence · DOM evidence</div></div></div>
    <div class="pipeline-doc"><span>🔧</span><div><a href="../../PATCH_PROPOSAL.md" target="_blank">PATCH_PROPOSAL.md</a><div class="pdesc">Search/replace patch · risks · verification</div></div></div>
    <div class="pipeline-doc"><span>🔀</span><div><a href="../../PR_SUMMARY.md" target="_blank">PR_SUMMARY.md</a><div class="pdesc">GitHub PR description · problem · fix · risks</div></div></div>
    <div class="pipeline-doc"><span>💬</span><div><a href="../../COMMIT_MESSAGE.md" target="_blank">COMMIT_MESSAGE.md</a><div class="pdesc">Conventional commit · ready to copy-paste</div></div></div>
    <div class="pipeline-doc"><span>✅</span><div><a href="../../REVIEW_CHECKLIST.md" target="_blank">REVIEW_CHECKLIST.md</a><div class="pdesc">Manual · regression · approval checklist</div></div></div>
  </div>
  <div class="pipeline-meta">
    Suggested branch: <span class="pipeline-branch">${pipeline.branch ?? '—'}</span>
    &nbsp;·&nbsp; ${pipeline.failureCount ?? 0} failure(s) in W${(pipeline.affectedWfs ?? []).join(', W')}
    &nbsp;·&nbsp; commit <code>${pipeline.commit ?? '—'}</code>
  </div>
</div>` : ''}
