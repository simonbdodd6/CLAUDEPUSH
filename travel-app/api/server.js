...
export function createHttpServer(apiOptions = {}) {
  ...
  async function route(req, body, token, url) {
    ...
    if (m === 'GET' && pathname === '/statistics') return api.getStatistics(token);
    if (m === 'GET' && pathname === '/insights') return api.getInsights(token);
    if (m === 'GET' && pathname === '/highlights') return api.getHighlights(token);
